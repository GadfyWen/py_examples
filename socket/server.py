...
while True:
  ...
  for fd, event in events:
     print "{}--{}--{}".format(event, select.EPOLLIN, select.EPOLLHUP)
     ...
     if socket == serversocket:
            ...
            recv_buffers[connection]  = ""
            messagers[connection] = messager.Messager()
     #关闭事件
     ...
     #可读事件
        ...
        recv_buffers[connection] += data
        if not messagers[connection].type:
            messagers[connection].unpack(recv_buffers[connection])
        if len(recv_buffers[connection]) < messagers[connection].len + messager.HEADER_SIZE:
            continue
        else:
